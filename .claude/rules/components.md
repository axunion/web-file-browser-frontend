...
- Icon data is bundled, not fetched from the Iconify API at runtime: `src/constants/icons.ts`
  is generated by `pnpm generate:icons` (which fetches only the referenced icons from the
  Iconify API, so no `@iconify-json/*` packages are needed) and registered at startup
  (`main.tsx`) and in the test setup. After adding or removing an icon name, rerun
  `pnpm generate:icons` — `src/constants/icons.test.ts` fails if the bundle is stale.
